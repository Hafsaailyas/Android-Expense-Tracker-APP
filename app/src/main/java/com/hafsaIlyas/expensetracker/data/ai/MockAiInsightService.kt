...
class MockAiInsightService @Inject constructor() : AiInsightService {
    ...
    override suspend fun generateInsights(
        ...
        currency      : Currency,        // ✅ added — replaces hardcoded Locale.US formatter
        monthlyBudget : Double
    ): InsightResult {

        delay(2_200)   // simulate network latency so the loading state is visible

        // ✅ All monetary formatting now uses the user's chosen currency
        ...
        insights += Insight(
            ...
            body     = buildString {
                ...
                append("you are on track to spend approximately ${fmt.format(projected)} by the end of the month. ")
                if (monthlyBudget > 0) {
        ...
        return InsightResult(
            summary  = "Analysed ${expenses.size} transactions totalling ${fmt.format(total)}",
            insights = insights
        )
    }
}